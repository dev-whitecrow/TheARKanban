        ...
        if (isExternalChange) {
            ...
            if (existingTask) {
                ...
                if (o.status !== n.status)
                    changes.push(`status: ${o.status} → ${n.status}`);
                if (o.priority !== n.priority)
                    changes.push(`priority: ${o.priority} → ${n.priority}`);
                if (o.assignee !== n.assignee)
                    changes.push(`assignee: ${o.assignee ?? 'unassigned'} → ${n.assignee ?? 'unassigned'}`);
                if (o.title !== n.title)
                    changes.push(`title → ${n.title}`);
                if (o.dueDate !== n.dueDate)
                    changes.push(`dueDate → ${n.dueDate ?? 'cleared'}`);
                if (o.epic !== n.epic)
                    changes.push(`epic → ${n.epic ?? 'cleared'}`);
                if (existingTask.body !== task.body)
