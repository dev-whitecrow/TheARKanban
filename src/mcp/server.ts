...
import { TaskStatus, TaskPriority, isValidTransition, ALLOWED_TRANSITIONS } from '../schema.js';
...
async function main() {
  ...
  server.tool(
    'get_board_state',
    'Get the full Kanban board state, grouped by columns with task counts.',
    {},
    async () => {
  ...
  server.tool(
    'list_tasks',
    'List tasks with optional filtering by status and/or assignee.',
    {
    ...
    async ({ status, assignee }) => {
      ...
      if (status) tasks = tasks.filter((t) => t.frontmatter.status === status);
      if (assignee) tasks = tasks.filter((t) => t.frontmatter.assignee === assignee);
  ...
  server.tool(
    'move_task',
    'Move a task to a new status column. Validates allowed transitions.',
    {
  ...
  server.tool(
    'add_note',
    'Add a note to a task\'s Activity Log. Use this to record progress, decisions, or observations.',
    {
